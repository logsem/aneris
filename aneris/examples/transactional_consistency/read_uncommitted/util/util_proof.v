...
Section proof.
  ...
  Context `{!anerisG Mdl Σ, !User_params, !KVS_transaction_api, 
            !RU_resources Mdl Σ, !RU_client_toolbox}.
    ...
    - wp_pures.
      ...
      + rewrite /commitU.
        ...
        * iNext.
          ...
          iApply ("HΦ" $! (V' ∖ {[v]})).
          iFrame.
          assert (V' = {[v]} ∪ (V' ∖ {[v]})) as <-; last iFrame.
          assert (v = v') as ->; first by apply Hbeq.
          rewrite -union_difference_singleton_L; done.
      + iApply ("IH" with "[$][$][$]").
    - wp_pures.
      iApply ("IH" with "[$][$][$]").
  Qed.
