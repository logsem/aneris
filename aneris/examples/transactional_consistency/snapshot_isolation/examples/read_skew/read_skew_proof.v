...
From aneris.examples.transactional_consistency.snapshot_isolation.examples.read_skew
  Require Import read_skew_code.
Import ser_inj.
...
Context `{!anerisG Mdl Σ, !SI_resources Mdl Σ, !KVSG Σ}.
  ...
  Proof.
    ...
    try iDestruct (token_exclusive with "Htok Htok'") as "[]".
    iModIntro.
    iExists {["x" := []; "y" := []]} , _, {["x" := (Some #1, true); "y" := (Some #1, true)]}.
    iFrame.
