...
From aneris.examples.transactional_consistency.snapshot_isolation.examples.write_skew
  Require Import write_skew_code.
Import ser_inj.
    ...
    - iExists {["x" := []; "y" := hy']} , _, {["x" := (Some #1, true); "y" := (last hy, false)]}.
      ...
      iSplitL "Hcx1 Hcx2 Hcy1 Hcy2 Hkx Hky".
        ...
        + iNext. iIntros "[_ HBig]".
          iMod ("Hclose" with "[Hhyp_y HBig Htok]") as "_".
              * iNext. iExists _, _.
                rewrite !(big_sepM2_insert); try set_solver.
                iDestruct "HBig" as "((Hkx & _) & (Hky & _) & _)".
                iFrame.
                iSplitL "Htok".
                iRight.
                iFrame.
                by iPureIntro.
                iDestruct "Hhyp_y" as "[-> | (-> & Htoky)]".
                iLeft.
                iPureIntro.
                destruct (last hy); done.
                iRight.
                iFrame.
                iPureIntro.
                destruct (last hy); done.
    ...
    - iExists {["x" := hx'; "y" := []]} , _, {["x" := (last hx, false); "y" := (Some #1, true)]}.
      ...
      iSplitL "Hcx1 Hcx2 Hcy1 Hcy2 Hkx Hky".
        + iSplitR. iPureIntro. set_solver.
          ...
          rewrite !big_sepM_empty. iFrame.
        + iNext. iIntros "[_ HBig]".
          iMod ("Hclose" with "[Hhyp_x HBig Htok]") as "_".
              * iNext. iExists _, _.
                rewrite !(big_sepM2_insert); try set_solver.
                iDestruct "HBig" as "((Hkx & _) & (Hky & _) & _)".
                iFrame.
                iSplitL "Hhyp_x".
                iDestruct "Hhyp_x" as "[-> | (-> & Htokx)]".
                iLeft.
                iPureIntro.
                destruct (last hx); done.
                iRight.
                iFrame.
                iPureIntro.
                destruct (last hx); done.
...
Context `{!anerisG Mdl Σ, !SI_init, !KVSG Σ}.
  ...
  Proof.
    ...
    first set_solver.
    iDestruct (big_sepS_delete _ _ "y" with "mem") as "(Hy & HKVSres)";
