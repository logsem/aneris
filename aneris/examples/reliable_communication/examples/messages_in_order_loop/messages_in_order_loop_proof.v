...
Section proof_of_the_code.
  ...
  Proof.
    ...
    wp_smart_apply (RCSpec_make_server_skt_spec with "[$Hmh $Hfp $Hsi $Hit][HΦ]").    iNext. iIntros (skt) "Hcl". wp_pures.
  ...
  Lemma wp_client_carpe_diem clt_addr (s1 s2 : string) :
    ...
    {{{ skt, RET skt; True }}}.
  Proof.
    iIntros (Φ) "(#Hsi & Hmh & Hfp & Hf) HΦ".  rewrite /client_0.
...
Section proof_of_the_main.
  ...
  Proof.
    ...
    iIntros "".
    iModIntro.
    iIntros "#Hsrv Hf Hfree1 Hfree2 Hfree3 Hsa1 Hsa2 Hsa3 HSrvInit".
    rewrite /main.
    wp_apply aneris_wp_start; first done.
    iFrame "Hfree1".
    iSplitR "Hsa1 HSrvInit"; last first.
    { iNext. iIntros "Hfps".
      iApply (@wp_server _ _ _ _ _ SnRes chn _ with "[-]"); last done.
    by iFrame "#∗". }
    iNext. wp_pures.
    wp_apply aneris_wp_start; first done.
    iFrame "Hfree2".
    iDestruct (unallocated_split with "Hf") as "[Hf0 Hf1]"; [set_solver|].
    iSplitL "Hfree3 Hsa3 Hf1".
    - iNext. wp_pures. wp_apply aneris_wp_start; first done.
      + iFrame. iSplit; first done.
        iNext. iIntros "Hfps".
        iApply (@wp_client _ _ _ _ _ SnRes chn _ _ clt_sa1 with "[-]"); last done.
        iFrame "#∗".
    - iNext. iIntros "Hfi1".
      iApply (@wp_client _ _ _ _ _ SnRes chn _ _ clt_sa0 with "[-]"); last done.
      iFrame "#∗".
  Qed.
