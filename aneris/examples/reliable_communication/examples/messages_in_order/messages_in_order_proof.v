...
From aneris.examples.reliable_communication.examples.messages_in_order
     Require Import messages_in_order_code.
...
Section Proto.
  ...
  Definition proto_in_order : iProto Σ :=
    (<! (m1 : Z) > MSG #m1 ;
     <! (m2 : Z) > MSG #m2 ;
     <! (m3 : Z) > MSG #m3 ;
     <?> MSG #m1;
...
Section proof_of_the_code.
  ...
  {| RCParams_clt_ser := int_serialization;
    ...
    RCParams_clt_ser_inj := ser_inj.int_ser_is_ser_injective;
 ...
 Lemma wp_server :
    ...
    {{{ skt, RET skt; True }}}.
  Proof.
    iIntros (Φ) "(#Hsi & Hmh & Hfp & Hit) HΦ". rewrite /server. wp_pures.
  ...
  Lemma wp_client clt_addr :
    ...
    {{{ skt, RET skt; True }}}.
  Proof.
    iIntros (Φ) "(#Hsi & Hmh & Hfp & Hf) HΦ". rewrite /client. wp_pures.
    ...
    simpl in *. rewrite /proto_in_order. wp_pures.
    wp_send with "[//]". wp_send with "[//]". wp_send with "[//]". wp_pures.
    wp_recv as "_". wp_recv as "_". wp_recv as "_". do 2 wp_pure _.
    wp_apply wp_assert. wp_pures. iSplit; first done. by iApply "HΦ".
  Qed.
...
Proof.
  ...
  simpl in *.
  iDestruct (unfixed_split with "Hf") as "[Hf_srv Hf]"; [set_solver|].
