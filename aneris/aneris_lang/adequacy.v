...
Proof.
  ...
  iMod (unfixed_init A) as (γsif) "[Hunfixed_auth Hunfixed]".
  ...
  iMod (socket_address_group_own_alloc_subseteq _ A A with "Hauth") as
      "[Hauth HownA]"; [done|].
  ...
  set (dg :=
         {|
           ...
           aneris_unfixed_socket_address_groups_name := γsif;
  ...
  iMod (socket_address_group_own_alloc_subseteq _ A obs_send_sas with "Hauth")
    as "[Hauth Hown_send]"; [done|].
  iDestruct (socket_address_group_own_big_sepS with "Hown_send") as "Hown_send".
  iMod (socket_address_group_own_alloc_subseteq _ A obs_rec_sas with "Hauth")
    as "[Hauth Hown_recv]"; [done|].
  iDestruct (socket_address_group_own_big_sepS with "Hown_recv") as "Hown_recv".
  iSplitR.
  { eauto. }
  iSplitR "Hwp Hunfixed HIPs HB Hmfrag Halobs Hsendevs Hreceiveevs Hown_send Hown_recv"; last first.
  {
    iDestruct ("Hwp" with "Hunfixed HB [$Hmfrag //] HIPs Hn Halobs [Hsendevs Hown_send] [Hreceiveevs Hown_recv] Hobserved_send Hobserved_receive") as "Hwp".
    ...
    iIntros "!> ((?&?&?&%&?) &?) /=". iFrame. done. }
  iMod (socket_address_group_own_alloc_subseteq _ A (obs_send_sas ∪ obs_rec_sas) with "Hauth")
  ...
  iMod (socket_address_group_own_alloc_subseteq _ A A with "Hauth")
    as "[Hauth Hown]"; [by set_solver|].
  iPoseProof (@aneris_state_interp_init _ _ dg IPs
               with "Hmp [//] Hh Hs Hms [$Hauth $Hown] Hunfixed_auth Hsi HIPsCtx HPiu") as "$"; eauto.
  simpl.
...
Theorem adequacy1 `{anerisPreG Σ Mdl} `{EqDecision (aneris_to_trace_model Mdl)} IPs A
  ...
  adequate s (mkExpr ip e) σ (λ v _, φ v).
Proof.
  intros HMdlfin Hwp Hsendle Hrecvle Hipdom Hpiiu Hip Hfixdom Hste Hsce Hmse.
