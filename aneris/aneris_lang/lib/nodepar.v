...
Section proof.
  ...
  Lemma par_spec Φ1 Φ2 ip1 ip2 e1 e2 :
    Φ1 ∗ Φ2 ∗
    free_ip ip1 ∗ free_ip ip2 ∗
    ((Φ1 -∗ WP e1 @[ip1] {{ _, True }}) ∗
     (Φ2 -∗ WP e2 @[ip2] {{ _, True }}))%I ⊢
    WP ((ip1; e1) ||| (ip2; e2)) @["system"] {{ _, True }}.
  Proof.
    iIntros "(HΦ1 & HΦ2 & HIP1 & HIP2 & Hwp1 & Hwp2)".
    wp_apply (aneris_wp_start with "[-]"). iFrame.
    iSplitR "HΦ1 Hwp1"; last first.
    { iNext. iApply ("Hwp1" with "HΦ1"). }
    iNext. simpl.
    wp_apply (aneris_wp_start with "[-]"). iFrame.
    iSplitR "HΦ2 Hwp2"; last first.
    { iNext. iApply ("Hwp2" with "HΦ2"). }
    iNext. by wp_pures.
  Qed.
