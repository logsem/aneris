...
(*   (* TODO: separate these conditions and lifting premise *)
(*      into three statements corresponding to three types of steps *) *)
(*   Definition nice_step *)
(*     (c1 c2: cfg Λ) (oζ: olocale Λ) δ1 δ2 (ℓ': lm_lbl iLM) := *)
(*     (exists ζ U, oζ = Some ζ /\ ls_tmap δ2 (LM := iLM) = <[ζ := U]> (ls_tmap δ1 (LM := iLM))) \/ *)
(*       True \/ *)
(*       True.  *)

(*   Let LM_step_lifting_def: iProp Σ :=  *)
(*       ∀ c1 δ1, em_msi c1 δ1 (em_GS0 := eGS) -∗ *)
(*                ∃ δ1', partial_msi δ1' ∗ *)
(*                       ∀ (c2: cfg heap_lang) δ2' oζ ℓ', *)
(*                         (* ⌜ valid_evolution_step oζ c2 δ1' ℓ' δ2' (LM := iLM)⌝ -∗ *) *)
(*                         ⌜ lm_ls_trans iLM δ1' ℓ' δ2'⌝ -∗ *)
(*                         ⌜ locale_step c1 oζ c2 ⌝ -∗ *)
(*                         ⌜ nice_step c1 c2 oζ δ1' δ2' ℓ' ⌝ -∗ *)
(*                         partial_msi δ2' -∗ *)
(*                         ∃ δ2 ℓ, em_msi c2 δ2 (em_GS0 := eGS) ∗ *)
(*                                 ⌜ em_valid_evolution_step oζ c2 δ1 ℓ δ2⌝. *)
...
(*                lm_ls_trans iLM δ1' (Silent_step ζ) δ2' ⌝ ∗ *)
(*              has_fuels ζ (filter (λ '(k, _), k ∈ dom fs ∖ ∅) fs) ∗ *)
(*              partial_msi δ2')%I. *)
...
(*       : iProp Σ := *)
(*     ( has_fuels ζ fs1 -∗ partial_model_is s1 -∗ partial_msi δ1' -∗ *)
(*     partial_free_roles_are fr1 *)
(*     ={ Einvs }=∗ *)
(*     ∃ (δ2': iLM) ℓ', *)
(*       ⌜ *)
...
(*         valid_evolution_step (Some ζ) (tp2, σ2) δ1' ℓ' δ2' (LM := iLM) *)
(*          ⌝ ∗ *)
(*       has_fuels ζ fs2 ∗ partial_model_is s2 ∗ partial_msi δ2' ∗ *)
(*       partial_free_roles_are (fr1 ∖ (live_roles _ s2 ∖ live_roles _ s1) ∪ fr_stash)).  *)
...
(*           (∀ δ fr fs tid, partial_free_roles_fuels_disj_def δ fr fs tid) ∗ *)
(*           LM_step_lifting_def *)
(* ).   *)
...
(*     Proof. by iIntros "(?&?&?&?&?)". Qed. *)
(*     Lemma partial_free_roles_fuels_disj {Einvs} δ fr fs tid: *)
(*       PartialModelPredicates Einvs ⊢ partial_free_roles_fuels_disj_def δ fr fs tid. *)
(*     Proof. by iIntros "(?&?&?&?&?)". Qed. *)
