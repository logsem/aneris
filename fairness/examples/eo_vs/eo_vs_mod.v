...
Section Models.
  ...
  Proof.
    { exact (list_to_set ∘ extract_Somes ∘ elements ∘ proj1_sig prod_AM_strong_lr). }
    intros. rewrite elem_of_list_to_set.
    apply elem_of_list_In. apply extract_Somes_spec.
    apply elem_of_list_In. apply elem_of_elements.
    destruct prod_AM_strong_lr as [f SPEC].
    simpl. apply SPEC. eauto.
  Defined. 
