 ...
 |}.
  ...
  Proof.
    ...
    { eapply ev_rel_after in EV_REL; eauto.
      clear -EV_REL UPTOi.
      rename mtr_i' into mtr. rename lmtr_i into lmtr. rename UPTOi into UPTO. 
      red in EV_REL. 
      red. intros.
      forward eapply upto_state_lookup_unfold1 as (mtr_i' & i & lmtr_i & AFTERi & AFTERi' & UPTOi & ITH'); eauto.
      
      assert (∀ g : G, fair_by_group (ELM_ALM LM_EM_EXT_KEEPS) g lmtr_i) as FAIR. 
      { admit. }
      ...
      { destruct (decide (ls_mapping (trfirst lmtr_i) !! ρ' = Some (asG ρ'))).
        ...
        apply group_fairness_implies_step_or_unassign with (ρ := ρ') in FAIR.
        2: done. 
        apply fair_by_gen_equiv in FAIR.
        red in FAIR. specialize (FAIR 0). specialize_full FAIR.
        { erewrite state_lookup_after; eauto.
          rewrite Nat.add_0_r ITH. simpl.
          eapply elem_of_dom; eauto. }
        simpl in FAIR. destruct FAIR as [m_ FAIR]. 
        pattern m_ in FAIR. eapply min_prop_dec in FAIR as [m [FAIR MIN]].
        2: { admit. }
        destruct FAIR as (δ_m & step_m & MTH & FAIR).
        red in FAIR.
        ...
        { eapply steps_keep_state_gen. 
          ...
          { (* TODO: move *)
            Unshelve.
            2: exact (fun δ1 oℓ δ2 => match oℓ with 
                                   | Some (inl g) => lm_fair_traces.next_TS_role δ1 g δ2 ≠ Some ρ'
                                   | oℓ => other_proj (asG ρ') oℓ
                                   end). 
            (* 2: exact (other_proj (asG ρ')).  *)
            ...
            - destruct ι; simpl in REL; red in REL.
              + destruct ρ0 as [ρ0]. 
                ...
                { split; [admit| ].
                  apply P1. }
                2: { simpl. right. Unshelve. 2: exact (flU ρ0).
                     apply REL. }
                red. simpl.
              ...
              + destruct ρ0 as [ρ0]. 
                ...
                { split; [admit| ].
                  apply P1. }
                2: { simpl. right. Unshelve. 2: exact (flL ρ0).
                     apply REL. }
                red. simpl.
                intros ->. simpl in PSTEP. congruence. }
          { intros * [_ ?] KTH.
            destruct oℓ'; [| done]. simpl. 
            (* enough (m <= k); [lia| ]. apply MIN.      *)
            (* apply trace_label_lookup_simpl' in KTH as (δ1&?&KTH). *)
