...
Section FairLockLM.
 ...
 Hypothesis DROP_PRES_LSI: forall st rem, fuel_drop_preserves_LSI st rem (LSI := LSI). 
  ...
  refine {|
      fair_lock.au_impl := au_impl;
      fair_lock.al_impl := al_impl;
...
(exists i' st', i <= i' /\
  ...
  Proof.
    ...
    inversion STEP; subst.
    -
      assert (does_lock ρ (ls_under δ') /\ fair_lock.disabled_st ρ (ls_under δ') /\ ls_tmap δ' !! asG ρ = Some ∅) as (?&?&?).
  ...
  Proof.
    ...
    inversion STEP; subst.
    -
      assert (does_unlock ρ (ls_under δ') /\ fair_lock.disabled_st ρ (ls_under δ') /\ ls_tmap δ' !! asG ρ = Some ∅) as (?&?&?).
      { simpl in STEP0, LOCK, DIS.
        simpl in OTHER.  destruct ρ0 as [ρ'].
        unfold_LMF_trans STEP0.
        2: { simpl in STEP1. pose proof STEP1 as SS. repeat apply proj2 in STEP1.
  ...
  Lemma final_steps ρ i (P P': R → fmstate M → Prop) flc
    ...
    ∃ (n : nat) (st' : fmstate ext_model_FM),
      i <= n
      ∧ lmtr S!! n = Some st'
      ∧ lift_prop1 P P' (asG ρ) st' ∧ fair_lock.disabled_st (asG ρ) st'.
  Proof.
  ...
  - intros δ [ρ]. simpl.
    destruct (decide (asG ρ ∈ dom (ls_tmap δ))); [| tauto].
    apply or_assoc. left.
    pose proof (fair_lock.does_lock_unlock_trichotomy (ls_under δ) ρ) as T.
    apply or_assoc in T as [T | U].
    2: { by apply UNUSED_NOT_DOM in U. }
    destruct (decide (role_enabled_model ρ (ls_under δ))), (decide (role_enabled_model ((asG ρ): fmrole LMF) δ)); tauto.
  Qed. 
