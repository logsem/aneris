...
Section fairness_preserved.
  ...
  Definition fair_ex_gen ζ (extr: extrace Λ): Prop :=
    forall n, pred_at extr n (λ c _, locale_prop ζ c) ->
         ∃ m, pred_at extr (n+m) (λ c _, ¬locale_prop ζ c)
  ...
  (* TODO: rename *)
  Definition lm_exaux_traces_match_gen: extrace Λ → auxtrace (LM := LM) → Prop :=
    traces_match 
    labels_match
    (* live_tids *) state_rel
    locale_step 
  ...
  Lemma match_locale_prop (extr : extrace Λ) (auxtr : auxtrace (LM := LM)) ζ ρ:
  ...
  Definition fairness_induction_stmt ρ fm f m ζ extr (auxtr : auxtrace (LM := LM))
    ...
    :=
      (infinite_trace extr ->
       ...
       (pred_at extr m (λ c _, ¬locale_prop ζ c) ∨ pred_at extr m (λ _ oζ, oζ = Some (Some ζ))) ->
  ...
  Local Lemma case1 ρ f m (extr' : extrace Λ) (auxtr' : auxtrace (LM := LM)) δ ℓ :
    ...
    Lemma fair_ex_gen_cons :
∀ (tid : locale Λ) (c : cfg Λ) (tid' : olocale Λ) (r : trace (cfg Λ) (olocale Λ)),
  fair_ex_gen tid (c -[ tid' ]-> r) → fair_ex_gen tid r.
    Proof. Admitted. 
  ...
  Proof.
    ...
    have Hfair': (forall ζ, fair_ex_gen ζ extr').
    { intros. by eapply fair_ex_gen_cons. }
...
(*            impossible because tid and the label must match! *)
    ...
    - (* Another thread is taking a step. *)
      ...
      + have [f' [Hfuel' Hff']] : exists f', ls_fuel (trfirst auxtr') !! ρ = Some f' ∧ f' ≤ f.
        ...
        unfold fair_ex_gen in *.
        have Hζ'en: pred_at extr' 0 (λ (c : cfg Λ) _, locale_prop ζ c).
        { rewrite /pred_at /= pred_first_trace.
          eapply match_locale_prop; eauto. 
          by inversion Htm. }
      ...
      + have [f' [Hfuel' Hff']] : exists f', ls_fuel (trfirst auxtr') !! ρ = Some f' ∧ f' < f.
        ...
        unfold fair_ex_gen in *.
        have: pred_at extr' 0 (λ c _, locale_prop ζ'' c).
        { rewrite /pred_at /= pred_first_trace.
          eapply match_locale_prop; eauto. 
          by inversion Htm. }
        have Hζ'en: pred_at extr' 0 (λ c _, locale_prop ζ'' c).
        { rewrite /pred_at /= pred_first_trace.
          eapply match_locale_prop; eauto. 
          by inversion Htm. }
  ...
  Lemma fair_ex_gen_after :
    ∀ (ζ : locale Λ) (tr tr' : trace (cfg Λ) (olocale Λ)) 
      (k : nat), after k tr = Some tr' → fair_ex_gen ζ tr → fair_ex_gen ζ tr'.
  Proof. Admitted.

  Theorem fairness_preserved (extr: extrace Λ) (auxtr: auxtrace (LM := LM)):
    infinite_trace extr ->
    lm_exaux_traces_match_gen extr auxtr ->
    (forall ζ, fair_ex_gen ζ extr) -> (forall ρ, fair_aux ρ auxtr (LM := LM)).
  Proof.
    ...
    have Hte: locale_prop ζ (trfirst tr1').
    { eapply match_locale_prop.
      1, 2: eauto.
      apply Hen. }
    
    setoid_rewrite pred_at_sum in Hex'. rewrite Heq' in Hex'.
    have Hpa: pred_at extr n (λ c _, locale_prop ζ c).
    ...
    intros ?. by eapply fair_ex_gen_after.
  Qed.
...
(* TODO: move? *)
Lemma match_locale_enabled_states_livetids `{EqDecision (locale Λ)}
  `{LM: LiveModel (locale Λ) M}
  ζ ρ δ c
...
Definition lm_exaux_traces_match `{EqDecision (locale Λ)} `{LM: LiveModel (locale Λ) M} :=
  lm_exaux_traces_match_gen live_tids (LM := LM). 
