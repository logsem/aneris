...
Section LMLSITopLevel. 
  ...
  Let ron_trivial (ζ g: locale heap_lang): iProp Σ := ⌜ ζ = g ⌝. 

  Lemma lm_lsi_toplevel:
    ⊢ LM_steps_gen ∅ (EM := @LM_EM_HL _ _ _ LF) (iLM := LM) (PMPP := ActualOwnershipPartialPre) (relies_on := ron_trivial) (eGS := fG). 
  Proof.
    ...
    - iModIntro. iIntros "* -> FUELS ST MSI FR". simpl in *.
      ...
      iModIntro. do 2 iExists _. iFrame. iPureIntro. split; [| split]. 
      + repeat split; eauto.
    ...
    - iIntros "* -> FUELS MSI". simpl in *.
      ...
      (* TODO: make it a lemma *)
      iAssert (⌜ (ls_tmap δ1 !! g = Some (dom fs)) ⌝)%I as %TMAPζ.
      ...
      do 3 iExists _. iFrame. iModIntro. iPureIntro. split.
      { done. }
      split; [done| ]. split. 
