...
From trillium.fairness.heap_lang Require Export tactics lm_lsi_hl_wp.
...
Lemma tac_wp_pure_helper `{EM: ExecutionModel heap_lang M} `{@heapGS Σ _ EM} 
  `{iLM: LiveModel G iM LSI} `{Countable G}
...
(* >>>>>>> origin/pmp_resource *)
  ...
  (LM_steps_gen Einvs (EM := EM) (iLM := iLM) (PMPP := PMPP) (eGS := heap_fairnessGS)) -∗ ( ▷^n (has_fuels tid fs -∗ WP (fill K e2) @ tid; E {{ Φ }})) -∗
...
Lemma tac_wp_pure_helper_2 `{EM: ExecutionModel heap_lang M} `{@heapGS Σ _ EM}
  ...
  φ →
  fuel_reorder_preserves_LSI (LSI := LSI) ->  
  (LM_steps_gen Einvs (EM := EM) (iLM := iLM) (PMPP := PMPP) (eGS := heap_fairnessGS)) -∗ ( ▷^n ((has_fuels tid ((λ m, m - n)%nat <$> fs)) -∗ WP (fill K e2) @ tid; E {{ Φ }})) -∗
  has_fuels tid fs -∗
  WP (fill K e1) @ tid; E {{ Φ }}.
Proof.
...
Lemma tac_wp_pure `{EM: ExecutionModel heap_lang M} `{@heapGS Σ _ EM}
  ...
  let LSG := (LM_steps_gen Einvs (EM := EM) (iLM := iLM) (PMPP := PMPP) (eGS := heap_fairnessGS)) in
  envs_entails Δ' (LSG -∗
...
Proof.
  ...
  intros ? PRES ? Δother Hlater Δ' LSG Hccl .
  iIntros "H #PMP".
...
Context `{iLM: LiveModel G iM LSI} `{Countable G}.
...
Proof using.
  intros ?? Hij ?.
  rewrite envs_entails_unseal=> Δother ?? PRES Δ' LSG Hccl.
  rewrite -wp_bind.
  iIntros "H #PMP".
  iAssert (⌜envs_wf Δ⌝)%I as %Hwf.
  { unfold of_envs, of_envs', envs_wf. iDestruct "H" as "[% _]". by iPureIntro. }
  ...
  (* iApply (wp_load_nostep with "[$] [H1 H2]"). *)
  iApply (wp_load_nostep with "PMP [H1 H2]"); [| iFrame |]; [by intros ?%fmap_empty_inv|].
...
Proof using.
  intros ?? Hij ?.
  rewrite envs_entails_unseal=> Δother ?? PRES LSG.
  ...
  rewrite has_fuels_gt_1 //.
  iApply (wp_store_nostep with "PMP [H1 H2]"); [| iFrame |]; [by intros ?%fmap_empty_inv|].
  iIntros "!> [Hl Hf]".
