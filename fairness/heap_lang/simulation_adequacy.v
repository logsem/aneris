...
Theorem strong_simulation_adequacy_general `(EM: ExecutionModel M)
  ...
  (∀ `{Hinv : @heapGS Σ M EM} ,
    ⊢ |={⊤}=>
       ...
       ={⊤}=∗
       WP e1 @ s; locale_of [] e1; ⊤ {{ v, em_thread_post 0%nat v (em_GS0 := heap_fairnessGS)}} ∗
       rel_always_holds0 R s state_interp (fun v => em_thread_post 0%nat v (em_GS0 := heap_fairnessGS)) e1 σ1 s1) ->
...
Proof.
  ...
  iMod (em_initialization _ s1 e1) as (fGS) "[LM_INIT MSI]"; [done| ].
  Unshelve. 2: by apply hPre. 

  set (distG := {| heap_fairnessGS := (fGS: (em_GS Σ (ExecutionModel := EM))) |}).
  iMod (H distG) as "Hwp". clear H.
 
  iExists state_interp, (fun v => em_thread_post 0%nat v), fork_post.
  iSplitR.
