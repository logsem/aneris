...
Definition lm_init `{LM: LiveModel heap_lang M} (δ: mstate LM) :=
...
refine
  {|
    ...
    em_init_state := lm_init;     

    em_init_resource Σ := fun {_: fairnessGS LM Σ} δ => LM_init_resource δ;
|}.
intros. iIntros. iMod (init_fairnessGS_LM _ s1 e1) as "[% [? X]]"; [done| ]. 
