...
Section nodup_specs.
  ...
  Lemma wp_receivefrom_set ip a φ h s R T addrs av (rcv : val) l :
    ...
    {{{ ms mv, RET mv;
          ...
          ([∗ set] n ∈ addrs, ∃ m, ⌜m ∈ ms⌝ ∗ ⌜m_sender m = n⌝ ∗
                                   ⌜m_destination m = a⌝ ∗ φ m) }}}.
  Proof.
    ...
    iAssert (∃ sndrs ms vsndrs vmsgs,
                lsndr ↦[ip_of_address a] vsndrs ∗
                lmsg  ↦[ip_of_address a] vmsgs ∗
                is_rcvset_log l (ms ∪ R) a ∗
                a ⤳ (ms ∪ R, T) ∗
                ⌜ms ## R⌝ ∗
                ⌜is_set _ sndrs vsndrs⌝ ∗
                ⌜is_set _ (gset_map (λ m, (m_body m, m_sender m)) ms) vmsgs⌝ ∗
                [∗ set] n ∈ sndrs, ∃ m, ⌜m ∈ ms⌝ ∗ ⌜m_sender m = n⌝ ∗
                                        ⌜m_destination m = a⌝ ∗ φ m)%I
      with "[Hmsg Hsndr Ha Hlog]" as "Hlogs".
    { iExists ∅, ∅, _, _. rewrite union_empty_l_L. iFrame.
      iSplit; [iPureIntro; set_solver|]. by iFrame "%". }
    clear Hsndrs vsndrs Hmsg vmsgs.
    iLöb as "IH" forall (R).
    wp_pures.
    iDestruct "Hlogs" as (sndrs msgs vsendrs vmsgs)
                           "(Hsndrs & Hmsgs & Hlog & Ha & % & % & % & Hms)".
    wp_load.
    wp_apply wp_set_equal; [auto|].
    iIntros ([] Hb); wp_if.
    { wp_load. iApply ("HΦ" $! msgs). rewrite Hb. by iFrame. }
    wp_apply ("Hrecv" with "[$Hh $Hφ $Ha $Hlog]").
    iIntros (m) "(Hh & Ha & Hm & Hlog & % & %)".
    wp_pures. wp_load.
    wp_apply (wp_set_add _ _ _ sndrs with "[//]").
    iIntros (vsndrs' Hsndrs'). wp_store.
    wp_load.
    wp_apply (wp_set_add msg_toval _ (_,_)  with "[//]").
    iIntros (vmsgs' Hmsgs').
    wp_store.
    iApply ("IH" with "Hh [HΦ]"); last first.
    { iExists ({[m_sender m]} ∪ sndrs), ({[m]} ∪ msgs),  vsndrs', vmsgs'.
      ...
      iSplitL "Hm".
      { iExists m. iFrame. iSplit; [|auto]. iPureIntro; set_solver. }
      iApply (big_sepS_impl with "Hms").
      iIntros "!#" (??). iDestruct 1 as (m') "(% & % & ?)".
      iExists m'. iFrame. iFrame "%". iPureIntro; set_solver. }
