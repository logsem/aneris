...
Section vect_specs.
  ...
  Proof.
    ...
    - destruct Hcoh as [k [H' Hl']].
      ...
      + wp_bind (list_tail _).
        iApply (wp_list_tail _ _ (#x :: (map (λ n : nat, #n) t))).
...
Definition vc_serialization : serialization :=
  {| DBS_valid_val := vc_valid_val;
     ...
     DBS_ser_spec := @wp_vect_serialize;
     DBS_deser_spec := @wp_vect_deserialize; |}.
