...
Section primitive_laws.
  ...
  Proof.
    ...
    iSplitR; [ iPureIntro; do 3 eexists; eapply LocalStepS; eauto | ].
    ...
    iModIntro. iSplitR.
     { iPureIntro.
       assert (r = ∅ ∨ ∃ m, m ∈ r) as [->| (m & Hm)];
         [ | do 3 eexists; by eapply SocketStepS; eauto; econstructor..].
  ...
  Lemma wp_receivefrom_gen k a E h s R T φ :
    let ip := ip_of_address a in
    saddress s = Some a →
  {{{ ▷ h ↪[ip] s ∗ ▷ a ⤳ (R, T) ∗ a ⤇ φ}}}
    (mkExpr ip (ReceiveFrom (Val $ LitV $ LitSocket h))) @ k; E
