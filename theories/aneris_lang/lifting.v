...
Section primitive_laws.
  ...
  Proof.
    ...
    by eapply message_history_evolution_new_socket.
    iSplitR; first done.
    iFrame. by iApply "HΦ".
  Qed.
 ...
 Lemma wp_socketbind_static A E sh skt k a :
  ...
  Proof.
    ...
    iPureIntro; split; last by left.
      by eapply message_history_evolution_socketbind.
      iSplitR; [done|]. iFrame.
    iApply ("HΦ" with "[$]").
  Qed.
  ...
  Proof.
    ...
    iModIntro. rewrite - /msg.
    iExists (AnerisAuxState
               ((aneris_AS_mhist δ).1, {[msg]} ∪ (aneris_AS_mhist δ).2)  _).
    iSplit. iPureIntro. split; last by left. simpl.
    eapply Hmhe. iFrame.
    iSplitR; [done|].
    iApply ("HΦ" with "[$]").
  Qed.
  ...
  Proof.
    ...
    iMod (aneris_state_interp_send_duplicate
            with "[$Hsh] [$Hrt] [$Hσ]")
      as "(%Hmhe & Hσ & Hsh & Hrt)"; [done..|]. rewrite - /msg.
       rewrite insert_id; last done.
       iModIntro.
       iExists δ.
       iSplit. iPureIntro. split; last by left. simpl.
       destruct (aneris_AS_mhist δ).
       eapply Hmhe. iFrame.
       iSplitR; [done|].
       simplify_eq /=.
       iFrame.
  ...
  Proof.
    ...
    - iNext; iModIntro.
      rewrite insert_id; last done.
      iExists δ; iSplit. iPureIntro. eapply valid_state_evolution_id.
      iFrame. iSplitR; [done|].
  ...
  {{{ m, RET (mkVal ip (SOMEV (PairV #(m_body m) #(m_sender m))));
   ...
   Lemma wp_receivefrom_hocap_gen k a E E' h s φ
         (P : iProp Σ)
   ...
   Proof.
     ...
     - destruct (decide (m ∈ R)) as [Hin | Hni ].
       + iNext. iMod "Hmk".
         ...
         simplify_eq /=. subst ip. eauto.
         iFrame.
         iApply wp_value. iApply "HΦ". iFrame. iExists _, _. iRight; eauto.
       + iPoseProof (aneris_state_interp_receive_some _ _ _ (Some φ)
            with "[Hsi] [$Hσ] [$Hsh] [$Ha] ")
           as (R') "(% & %Hst & Hrt & Hrest)"; [done..|].
         iDestruct "Hrt" as "[ ( % & -> & Hres ) | (% & %) ]"; last done.
  ...
  Proof.
    ...
    { iPureIntro.
      assert (r = ∅ ∨ ∃ m, m ∈ r) as [->| (m & Hm)].
      { destruct (decide (r = ∅));
          [ set_solver | by right; apply set_choose_L]. }
      - do 4 eexists; by eapply SocketStepS; eauto; econstructor.
      - do 4 eexists; by eapply SocketStepS; eauto; econstructor. }
      ...
      + iNext; iMod "Hmk" as "_".
        ...
        iExists
          (AnerisAuxState
             (R ∪ (aneris_AS_mhist δ1).1, (aneris_AS_mhist δ1).2)  _).
        iSplit.
        { simplify_eq /=; iPureIntro; split; [eauto|by left]. }
         iFrame. iSplit; last done.
         iApply wp_value; iApply "HΦ"; eauto with iFrame.
       + iPoseProof (aneris_state_interp_receive_some _ _ _ (Some φ)
            with "Hsi Hσ Hsh Ha ")
           as (R') "(% & %Hst & Hrt & Hrest)"; [done|done|done|done|].
         iDestruct "Hrt" as "[ ( % & -> & Hres ) | (% & %) ]"; last done.
         iNext; iMod "Hmk" as "_".
         iMod "Hrest" as "(Hσ & Hsh & Ha)".
         iModIntro.
        ...
        iSplit.
        { iPureIntro; split; [eauto|by left]. }
        iFrame. iSplit; last done.
        iApply wp_value; iApply "HΦ"; eauto with iFrame.
     - by rewrite Hblk in H2.
     - iNext. iMod "Hmk" as "_".
    ...
    - destruct (decide (m ∈ R)) as [Hin | Hni ].
      + iNext.
        iPoseProof (aneris_state_interp_receive_some _ _ _ (Some φ)
            with "[Hsi] [$Hσ] [$Hsh] [$Ha] ")
          as (R') "(% & %Hhst & Hrt & >(Hσ & Hsh & Ha))"; [done..|].
        iDestruct "Hrt" as "[ ( % & _ & _ ) | (% & ->) ]"; [done|].
        iFrame.
        iMod ("Hclose" with "[$Hsh Ha]") as "HΦ"; [auto|].
        iModIntro.
        iExists
          (AnerisAuxState
             (R ∪ (aneris_AS_mhist δ1).1, (aneris_AS_mhist δ1).2)  _).
      ...
      + iPoseProof (aneris_state_interp_receive_some _ _ _ (Some φ)
           ...
           as (R') "(% & %Hst & Hrt & Hrest)"; [done..|].
        iDestruct "Hrt" as "[ ( % & -> & Hres ) | (% & %) ]"; last done.
        iNext. iMod "Hrest" as "(Hσ & Hsh & Ha)".
        iMod ("Hclose" with "[$Hsh Ha Hres]") as "HΦ".
        { iLeft. iFrame. eauto. }
        iModIntro.
        iExists (AnerisAuxState
          ({[m]} ∪ R ∪ (aneris_AS_mhist δ1).1, (aneris_AS_mhist δ1).2) _).
        ...
        iFrame. iSplit; [|done].
        iApply wp_value. iApply "HΦ".
     - destruct s; simpl in *; simplify_eq.
     - iNext. iDestruct "Hclose" as "[Hclose _]".
       iMod ("Hclose" with "[$Hsh $Ha $Hsi]") as "HAU".
       iModIntro. iExists δ1; iSplit.
       { iPureIntro. rewrite insert_id //. eapply valid_state_evolution_id. }
       rewrite insert_id //; iFrame. iSplit; [|done].
       iApply ("IH" with "HAU").
  Qed.
