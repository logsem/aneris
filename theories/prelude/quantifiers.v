...
Fixpoint list_generated_by {A} (f : nat → A) (n : nat) : list A :=
  match n with
  | O => [f 0]
  | S n => f (S n) :: list_generated_by f n
  end.

Lemma list_generated_by_length {A} (f : nat → A) n :
  length (list_generated_by f n) = S n.
Proof. induction n as [|? IHn]; [|simpl; rewrite IHn]; done. Qed.

Lemma elem_of_list_generated_by {A} (f : nat → A) n x :
...
Lemma list_generated_by_NoDup {A} (f : nat → A) n :
...
Lemma finite_smaller_card_nat `{!EqDecision A} :
  Finite A → smaller_card A nat.
Proof.
