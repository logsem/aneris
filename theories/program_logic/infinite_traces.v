...
Definition inf_execution_trace Λ := inflist (cfg Λ).

Section inf_execution_trace.
  Context {Λ : language}.

  Definition inf_exec_prepend (c : cfg Λ)
  ...
  | valid_inf_exec_step ex c c' iex :
      valid_exec ex →
      exec_ends_in ex c →
      step c c' →
      valid_inf_exec (exec_extend ex c') iex →
      valid_inf_exec ex (inf_exec_prepend c' iex).
...
| valid_system_trace_step ex atr c c' δ δ' iex iatr :
    Ψ ex atr →
    exec_ends_in ex c →
    step c c' →
    auxtr_ends_in atr δ →
    ...
    valid_inf_system_trace
      Ψ (exec_extend ex c') (auxtr_extend atr δ') iex iatr →
    valid_inf_system_trace
      Ψ ex atr (inf_exec_prepend c' iex) (inf_auxtr_prepend δ' iatr).
...
Section simulation.
  ...
  Lemma valid_inf_exec_prepend_valid_exec_extend ex c iex :
    valid_inf_exec ex (inf_exec_prepend c iex) →
  ...
  Lemma produce_inf_aux_trace_next_aux_state_exists
        ...
        (Hvex : valid_inf_exec ex (inf_exec_prepend c iex)) :
    ∃ δ, continued_simulation φ (exec_extend ex c) (auxtr_extend atr δ).
  Proof.
  ...
  Definition produce_inf_aux_trace_next_aux_state
    ...
    : aux_state AS :=
    epsilon
      (produce_inf_aux_trace_next_aux_state_exists ex atr Hcsm c iex Hvex).
  ...
  Lemma produce_inf_aux_trace_next_aux_state_continued_simulation
    ...
    continued_simulation
      ...
      (exec_extend ex c)
      (auxtr_extend
         atr (produce_inf_aux_trace_next_aux_state ex atr Hcsm c iex Hvex)).
  Proof.
    rewrite /produce_inf_aux_trace_next_aux_state.
    apply epsilon_correct.
  Qed.

  Local Lemma valid_inf_exec_adjust {ex c iex} :
    valid_inf_exec ex (c :: iex)%inflist →
    valid_inf_exec (exec_extend ex c) iex.
  ...
  CoFixpoint produce_inf_aux_trace
    ...
    | c :: iex' =>
      λ Hvex',
      let δ :=
          produce_inf_aux_trace_next_aux_state ex atr Hcsm c iex' Hvex'
      in
      δ :: (produce_inf_aux_trace
              (exec_extend ex c)
              (auxtr_extend atr δ)
              (produce_inf_aux_trace_next_aux_state_continued_simulation
                 ex atr Hcsm c iex' Hvex')
              iex'
  ...
  Proof.
    ...
    - rewrite [produce_inf_aux_trace _ _ _ _ _]inflist_unfold_fold /=.
      pose proof (produce_inf_aux_trace_next_aux_state_continued_simulation
                    ex atr Hcsm c iex Hvex) as Hcsm'.
