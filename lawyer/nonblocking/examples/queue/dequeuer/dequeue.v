From iris.base_logic Require Export gen_heap.
From iris.proofmode Require Import tactics coq_tactics.
From iris.bi.lib Require Import fixpoint.
From trillium.program_logic Require Export weakestpre adequacy ectx_lifting.
From fairness Require Import utils.
From lawyer.examples Require Import obls_tactics.
From lawyer.obligations Require Import obligations_model obligations_resources obligations_am obligations_em obligations_logic env_helpers.
From lawyer Require Import sub_action_em program_logic.
From iris.algebra Require Import auth gmap gset excl excl_auth csum mono_nat.
From iris.base_logic.lib Require Import invariants.
From lawyer.nonblocking.examples.queue Require Import simple_queue_utils simple_queue.
From lawyer.nonblocking.examples.queue.dequeuer Require Import dequeuer_lib dequeue_ghost.
From heap_lang Require Import heap_lang_defs lang notation.


Close Scope Z.


Definition free_el: val :=
  λ: "nd",
    Free ("nd" +ₗ #0) ;;
    Free ("nd" +ₗ #1)
. 

Definition get_to_free (sq: SimpleQueue): val :=
  λ: "ph",
    if: ("ph" = !#BeingRead)
    then
      let: "old_fl" := !#FreeLater in
      #FreeLater <- "ph" ;;
      "old_fl"
    else "ph"
.

Definition dequeue (sq: SimpleQueue): val :=
  λ: "q",
    let: "c" := !#Head in
    if: ("c" = !#Tail)
    then NONE
    else
      let: "v" := get_val "c" in
      #OldHeadVal <- "v" ;;
      #Head <- (get_next "c") ;;
      let: "to_free" := get_to_free sq "c" in
      free_el "to_free" ;;
      (SOME "v")
.

Section DequeueLawyer.

  Context {DegO LvlO LIM_STEPS} {OP: OP_HL DegO LvlO LIM_STEPS}.
  Context `{EM: ExecutionModel heap_lang M}.
  Notation "'Degree'" := (om_hl_Degree). 
  Notation "'Level'" := (om_hl_Level).  

  Context {Σ} {OHE: OM_HL_Env OP EM Σ}.
  (* Existing Instance OHE.  *)
  Context {QL: QueueG Σ}.
  Context {SQT: SimpleQueueTokens Σ}.
  Context {q_sq: SimpleQueue}. 

  Context (d: Degree).
  Context (PE: val -> iProp Σ) {PERS_PE: forall v, Persistent (PE v)}. 

  Let hGS: heap1GS Σ := iem_phys _ EM.
  Existing Instance hGS.

  Lemma get_head_next_spec τ π q h nd fl ph od:
    {{{ queue_inv PE ∗ ith_node h (ph, nd) ∗ dequeue_resources h fl ph od ∗
        th_phase_frag τ π q ∗ cp_mul π d get_loc_fuel }}}
      get_next #ph @ CannotFork; NotStuck; τ; ⊤
    {{{ RET #(nd.2); th_phase_frag τ π q ∗ dequeue_resources h fl ph od }}}.
  Proof using.
    simpl. iIntros (Φ) "(#INV & #HEADhn & DR & PH & CPS) POST".
    rewrite /get_next.
    destruct nd as [v nxt]. simpl.
    pure_steps.
    wp_bind (_ +ₗ _)%E.
    iApply sswp_MU_wp; [done| ].
    iApply sswp_pure_step; [done| ].
    MU_by_burn_cp. iApply wp_value.
    iMod (get_head_next_vs with "[$] [$] [$]") as "(NXT & NXT')". 
    iApply sswp_MU_wp; [done| ].
    iApply (wp_load with "NXT"). iIntros "!> NXT". 
    MU_by_burn_cp. iApply wp_value.
    iMod ("NXT'" with "[$]") as "DR".
    iModIntro. iApply "POST". iFrame.
  Qed.

  Lemma update_ohv_spec τ π q (v: val):
    {{{ queue_inv PE ∗ th_phase_frag τ π q ∗ cp_mul π d get_loc_fuel ∗ PE v }}}
      #OldHeadVal <- v @ CannotFork; NotStuck; τ; ⊤
    {{{ RET #(); th_phase_frag τ π q }}}.
  Proof using.
    iIntros (Φ) "(#INV & PH & CPS & PEv) POST".
    iApply wp_atomic. 
    iMod (update_ohv_vs with "[$] [$]") as "(%v0 & OHV & OHV')".
    iModIntro. 
    iApply sswp_MU_wp; [done| ].
    iApply (wp_store with "[$]"). iIntros "!> ?".
    MU_by_burn_cp. iApply wp_value.
    iMod ("OHV'" with "[$]") as "_". 
    by iApply "POST".
  Qed.

  Lemma dequeue_upd_head_spec τ π q h ph vh (nxh: loc) fl:
    {{{ queue_inv PE ∗
        ith_node h (ph, (vh, nxh)) ∗ 
        th_phase_frag τ π q ∗ cp_mul π d get_loc_fuel ∗
        dequeue_resources h fl ph None }}}
      #Head <- #nxh @ CannotFork; NotStuck; τ; ⊤
    {{{ RET #(); th_phase_frag τ π q ∗ dequeue_resources (h + 1) fl nxh (Some h) ∗
                   ∃ i r b, ith_read i r (h + 1) ∗ ⌜ r <= h ⌝ ∗
                               br_lb b ∗
                               (⌜ b < r ⌝ -∗ (ith_rp i rs_canceled ∨ ith_rp i rs_aborted ∨ ith_rp i (rs_proc (Some rsp_completed)))) }}}.
  Proof using PERS_PE.
    iIntros (Φ) "(#INV & #HTH & PH & CPS & DR) POST".
    iMod (dequeue_upd_head_vs with "[$] [$] [$]") as "(HEAD & HEAD')". 
    iApply sswp_MU_wp; [done| ].
    iApply (wp_store with "[$]"). iIntros "!> HEAD".
    MU_by_burn_cp. iApply wp_value.
    iMod ("HEAD'" with "[$]") as "(DR & X)". 
    iApply "POST". by iFrame.
  Qed.

  Definition dequeue_fuel := 100.

  Lemma check_BR_spec τ π q h fl ph ndh i r b0
    (READ_BOUND: r <= h):
    {{{ queue_inv PE ∗
        ith_node h (ph, ndh) ∗
        dequeue_resources (h + 1) fl ndh.2 (Some h) ∗ 
        th_phase_frag τ π q ∗ cp_mul π d get_loc_fuel ∗
        ith_read i r (h + 1) ∗ br_lb b0
    }}}
      ! #BeingRead @ CannotFork; NotStuck; τ; ⊤
    {{{ (pbr: loc), RET #pbr; th_phase_frag τ π q ∗
            dequeue_resources (h + 1) fl ndh.2 (if (decide (pbr = ph)) then Some h else None) ∗
            (⌜ pbr = ph ⌝ ∨ 
             ⌜ pbr ≠ ph ⌝ ∗ hn_interp (ph, ndh) ∗ PE ndh.1) ∗
            ∃ b' ndbr', br_lb b' ∗ ⌜ b0 <= b' ⌝ ∗ ith_node b' (pbr, ndbr')
    }}}.
  Proof using PERS_PE.
    iIntros (Φ) "(#INV & #HTH & DR& PH & CPS & #READ & #BR0) POST".
    iApply wp_atomic.
    iMod (check_BR_vs with "[$] [$] [$] [$] [$]") as "(%pbr & BR & BR')"; try done.
    iModIntro. 
    iApply sswp_MU_wp; [done| ]. 
    iApply (wp_load with "BR"). iIntros "!> BR".
    MU_by_burn_cp. iApply wp_value.
    iMod ("BR'" with "[$]") as "X". 
    iApply "POST". by iFrame.
  Qed.

  Lemma read_FL_spec τ π h q fl nd od:
  {{{ queue_inv PE ∗ dequeue_resources h fl nd od ∗
      cp π d ∗ th_phase_frag τ π q }}}
    ! #FreeLater @ CannotFork; NotStuck; τ; ⊤
  {{{ (pfl: loc), RET (#pfl);
      ∃ ndfl, ith_node fl (pfl, ndfl) ∗ 
      dequeue_resources h fl nd od ∗ th_phase_frag τ π q }}}.
  Proof using.
    iIntros (Φ) "(#INV & DR & CPS & PH) POST".
    iApply wp_atomic.
    iMod (read_FL_vs with "[$] [$]") as "(%pfl & FL & FL')". iModIntro. 
    iApply sswp_MU_wp; [done| ].     
    iApply (wp_load with "FL"). iIntros "!> FL".
    rewrite cp_mul_1. MU_by_burn_cp. iApply wp_value.
    iMod ("FL'" with "[$]") as "X".
    iModIntro. iApply "POST". iFrame. 
    iApply "X". 
  Qed.

  Lemma get_to_free_spec 
    τ π q h fl (ph: loc) ndh i r b
    (READ_BOUND: r <= h):
    {{{ queue_inv PE ∗
        ith_node h (ph, ndh) ∗
        dequeue_resources (h + 1) fl ndh.2 (Some h) ∗ 
        th_phase_frag τ π q ∗ cp_mul π d (get_loc_fuel + get_loc_fuel + get_loc_fuel) ∗
        ith_read i r (h + 1) ∗
        br_lb b ∗ (⌜ b < r ⌝ -∗ (ith_rp i rs_canceled ∨ ith_rp i rs_aborted ∨ ith_rp i (rs_proc (Some rsp_completed))))
    }}}
      get_to_free q_sq #ph @ CannotFork; NotStuck; τ; ⊤
    {{{ (to_free: loc), RET #to_free;
        ∃ hn fl', hn_interp (to_free, hn) ∗ th_phase_frag τ π q ∗
                    dequeue_resources (h + 1) fl' ndh.2 None ∗ PE ndh.1 }}}.
  Proof using PERS_PE.    
    simpl.
    iIntros (Φ) "(#INV & #HTH & DR& PH & CPS & #READ & #BR0 & #NO_FL) POST".
    rewrite /get_to_free. 
    pure_steps. 
    
    split_cps "CPS" get_loc_fuel.
    wp_bind (! _)%E. 
    iApply (check_BR_spec with "[-POST CPS]").
    { apply READ_BOUND. }
    { iFrame "#∗". }
    iIntros "!> %pbr (PH & DR & RES)".
    iDestruct "RES" as "(RES & %b1 & %ndbr1 & #BR1 & %LEb & #BRTH1)". 

    wp_bind (_ = _)%E.
    iApply sswp_MU_wp; [done| ].
    iApply sswp_pure_step.
    { set_solver. }
    MU_by_burn_cp. iApply wp_value.

    iDestruct "RES" as "[%PTR_EQ | (%NEQ & HNI & PEv)]".
    2: { rewrite bool_decide_false; [| set_solver]. rewrite decide_False; [| done].
         pure_steps. iApply "POST". iFrame. }

    rewrite PTR_EQ. 

    rewrite bool_decide_true; [| set_solver]. rewrite decide_True; [| done].
    pure_steps.

    wp_bind (! _)%E.
    split_cps "CPS" 1. rewrite -cp_mul_1.
    iApply (read_FL_spec with "[-POST CPS]").
    { iFrame "#∗". }
    iIntros "!> %pfl (%ndfl & #FLTH & DR & PH)".

    wp_bind (Rec _ _ _)%E. pure_steps.

    wp_bind (_ <- _)%E.
    iApply wp_atomic.
    iMod (upd_fl_vs with "[$] DR HTH READ BR0 NO_FL [$] [$] [$]") as "(FL & FL')"; eauto.
    iModIntro. 
    iApply sswp_MU_wp; [done| ].    
    iApply (wp_store with "FL"). iIntros "!> FL".
    MU_by_burn_cp. iApply wp_value.
    iMod ("FL'" with "[$]") as "(%hn & %fl' & HNF & DR & PEh)".
    iModIntro. wp_bind (Rec _ _ _)%E. pure_steps.
    iApply "POST". iFrame.
  Qed.

  Lemma free_el_spec τ π q ptr nd:
    {{{ th_phase_frag τ π q ∗ cp_mul π d (2 * get_loc_fuel) ∗ 
        hn_interp (ptr, nd) }}}
      free_el #ptr @ CannotFork; NotStuck; τ; ⊤
    {{{ v, RET v; th_phase_frag τ π q }}}.
  Proof using.
    simpl. iIntros (Φ) "(PH & CPS & HN) POST".
    destruct nd. iDestruct "HN" as "[L0 L1]". 
    rewrite /free_el.
    pure_steps.

    wp_bind (_ +ₗ _)%E.
    iApply sswp_MU_wp; [done| ].
    iApply sswp_pure_step; [done| ].
    MU_by_burn_cp. rewrite loc_add_0. iApply wp_value.

    wp_bind (Free _)%E.
    iApply sswp_MU_wp; [done| ].
    iApply (wp_free with "L0"). iIntros "!> FREE0". 
    MU_by_burn_cp. iApply wp_value.
 
    wp_bind (Rec _ _ _)%E. pure_steps. 

    wp_bind (_ +ₗ _)%E.
    iApply sswp_MU_wp; [done| ].
    iApply sswp_pure_step; [done| ].
    MU_by_burn_cp. iApply wp_value.

    wp_bind (Free _)%E.
    iApply sswp_MU_wp; [done| ].
    iApply (wp_free with "[$]"). iIntros "!> FREE1". 
    MU_by_burn_cp. iApply wp_value.

    by iApply "POST". 
  Qed.
  
  Lemma dequeue_spec (τ: locale heap_lang) (π: Phase) (q: Qp):
    {{{ queue_inv PE ∗ dequeue_token ∗ 
        th_phase_frag τ π q ∗ cp_mul π d dequeue_fuel }}}
      dequeue q_sq #() @ CannotFork; NotStuck; τ; ⊤
    {{{ (v: val), RET v; th_phase_frag τ π q ∗ dequeue_token ∗
                         (⌜ v = NONEV ⌝ ∨ ∃ v', ⌜ v = SOMEV v' ⌝ ∗ PE v') }}}.
  Proof using PERS_PE.
    simpl. iIntros (Φ) "(#INV & TOK & PH & CPS) POST".
    rewrite /dequeue. 
    pure_steps.

    wp_bind (! _)%E.
    iApply wp_atomic.
    iMod (dequeuer_read_head_vs with "[$] [$]") as "(%h & %ph & %fl & HEAD & HEAD')".
    iModIntro.
    iApply sswp_MU_wp; [done| ].
    iApply (wp_load with "HEAD"). iIntros "!> HEAD".
    MU_by_burn_cp. iApply wp_value.
    iMod ("HEAD'" with "[$]") as "DR". iModIntro. 
    
    wp_bind (Rec _ _ _)%E. pure_steps. 
    
    wp_bind (! _)%E.
    iApply wp_atomic.
    iMod (dequeuer_read_tail_vs with "[$] [$]") as "(%t & %br & %pt & TAIL & %ORDER & #HT & TAIL')".
    iModIntro. 
    iApply sswp_MU_wp; [done| ].
    iApply (wp_load with "[$]"). iIntros "!> TAIL".
    MU_by_burn_cp. iApply wp_value.
    iMod ("TAIL'" with "[$]") as "DR".
    iModIntro. 

    wp_bind (_ = _)%E.
    iApply sswp_MU_wp; [done| ].
    iApply sswp_pure_step.
    { set_solver. }
    MU_by_burn_cp. iApply wp_value.

    iDestruct "HT" as "[[%GE <-] | (%NEMPTY & %ndh & #HTH)]". 
    { assert (t = h) as ->.
      { red in ORDER. lia. }
      rewrite bool_decide_true; [| done].
      iMod (dequeuer_close with "[$] [$]") as "TOK".
      pure_steps. 
      iApply "POST". iFrame.
      by iLeft. }

    rewrite bool_decide_false; [| set_solver]. pure_steps.
    split_cps "CPS" get_loc_fuel; [cbv; lia| ].  
    iApply (get_head_val_spec with "[-POST CPS]").
    { done. }
    { iFrame "#∗". }
    iIntros "!> (PH & DR & PEh)".
    wp_bind (Rec _ _ _)%E. pure_steps.

    destruct ndh as [vh nxh]. simpl.
    wp_bind (_ <- _)%E.
    split_cps "CPS" get_loc_fuel; [cbv; lia| ].
    iApply (update_ohv_spec with "[$PH $CPS' $INV $PEh]").
    iIntros "!> PH".
    wp_bind (Rec _ _ _)%E. pure_steps.

    split_cps "CPS" get_loc_fuel; [cbv; lia| ].
    iApply (get_head_next_spec with "[-POST CPS]").
    { iFrame "#∗". }
    iIntros "!> [PH DR]". simpl.

    wp_bind (_ <- _)%E.
    split_cps "CPS" get_loc_fuel; [cbv; lia| ].
    iApply (dequeue_upd_head_spec with "[$CPS' $PH $INV $HTH $DR]").
    iIntros "!> (PH & DR & (%i & %r & %b & #ITHR & %BR & #BRB & #TOKS))".

    wp_bind (Rec _ _ _)%E. do 3 pure_step_cases.
    wp_bind (get_to_free _ _)%E.         
    split_cps "CPS" (3 * get_loc_fuel); [cbv; lia| ].
    iApply (get_to_free_spec with "[-POST CPS]").
    { apply BR. }
    { iFrame "#∗". }
    
    iIntros "!> %to_free (%hfr & %fl' & HNFR & PH & DR & PEv)".
    wp_bind (Rec _ _ _)%E. pure_steps.
    wp_bind (free_el _)%E.
    split_cps "CPS" (2 * get_loc_fuel); [cbv; lia| ].
    iApply (free_el_spec with "[CPS' PH HNFR]").
    { iFrame. }
    iIntros "!> %v PH".

    wp_bind (Rec _ _ _)%E. do 2 pure_step_cases. 
    iClear "TOKS ITHR HTH". 

    iClear "BRB". 
    clear dependent r t hfr to_free br ph b. 

    iMod (dequeuer_close with "[$] [$]") as "TOK". 
    pure_steps. iApply "POST". iFrame.
    iRight. iExists _. iSplit; [done| ]. by iFrame. 
  Qed.
    
End DequeueLawyer.


From lawyer.nonblocking Require Import pwp.
From lawyer.nonblocking.examples Require Import pwp_tactics. 

Section DequeuePwp.

  Context {Σ} {hG: heap1GS Σ} {invG: invGS_gen HasNoLc Σ}. 
  
  Context {QL: QueueG Σ}.
  Context {SQT: SimpleQueueTokens Σ}.
  Context {q_sq: SimpleQueue}.

  Context (PE: val -> iProp Σ) {PERS_PE: forall v, Persistent (PE v)}.

  Let iG := @irisG_looping _ HeapLangEM _ _ hG si_add_none.
  Existing Instance iG.

  Lemma get_head_next_pwp_spec τ h nd fl ph od:
    {{{ queue_inv PE ∗ ith_node h (ph, nd) ∗ dequeue_resources h fl ph od }}}
      get_next #ph @ CannotFork; NotStuck; τ; ⊤
    {{{ RET #(nd.2); dequeue_resources h fl ph od }}}.
  Proof using.
    simpl. iIntros (Φ) "(#INV & #HEADhn & DR) POST".
    rewrite /get_next.
    destruct nd as [v nxt]. simpl.
    pwp_pure_steps.
    wp_bind (_ +ₗ _)%E.
    iApply sswp_pwp; [done| ].
    iApply sswp_pure_step; [done| ].
    do 2 iModIntro.
    iApply wp_value.
    iApply wp_atomic. 
    iMod (get_head_next_vs with "[$] [$] [$]") as "(NXT & NXT')".
    iModIntro. 
    iApply sswp_pwp; [done| ].
    iApply (wp_load with "NXT"). iIntros "!> NXT".
    do 1 iModIntro. iApply wp_value.
    iMod ("NXT'" with "[$]") as "DR".
    iModIntro. iApply "POST". iFrame.
  Qed.

  Lemma update_ohv_pwp_spec τ (v: val):
    {{{ queue_inv PE ∗ PE v }}}
      #OldHeadVal <- v @ CannotFork; NotStuck; τ; ⊤
    {{{ RET #(); True }}}.
  Proof using.
    iIntros (Φ) "(#INV & PEv) POST".
    iApply wp_atomic. 
    iMod (update_ohv_vs with "[$] [$]") as "(%v0 & OHV & OHV')".
    iModIntro. 
    iApply sswp_pwp; [done| ].
    iApply (wp_store with "[$]"). iIntros "!> ?".
    do 1 iModIntro. iApply wp_value.
    iMod ("OHV'" with "[$]") as "_". 
    by iApply "POST".
  Qed.

  Lemma dequeue_upd_head_pwp_spec τ h ph vh (nxh: loc) fl:
    {{{ queue_inv PE ∗
        ith_node h (ph, (vh, nxh)) ∗ 
        dequeue_resources h fl ph None }}}
      #Head <- #nxh @ CannotFork; NotStuck; τ; ⊤
    {{{ RET #(); dequeue_resources (h + 1) fl nxh (Some h) ∗
                   ∃ i r b, ith_read i r (h + 1) ∗ ⌜ r <= h ⌝ ∗
                               br_lb b ∗
                               (⌜ b < r ⌝ -∗ (ith_rp i rs_canceled ∨ ith_rp i rs_aborted ∨ ith_rp i (rs_proc (Some rsp_completed)))) }}}.
  Proof using PERS_PE.
    iIntros (Φ) "(#INV & #HTH & DR) POST".
    iApply wp_atomic.
    iMod (dequeue_upd_head_vs with "[$] [$] [$]") as "(HEAD & HEAD')".
    iModIntro.
    iApply sswp_pwp; [done| ].
    iApply (wp_store with "[$]"). iIntros "!> HEAD".
    iModIntro. iApply wp_value.
    iMod ("HEAD'" with "[$]") as "(DR & X)". 
    iApply "POST". by iFrame.
  Qed.

  Lemma check_BR_pwp_spec τ h fl ph ndh i r b0
    (READ_BOUND: r <= h):
    {{{ queue_inv PE ∗
        ith_node h (ph, ndh) ∗
        dequeue_resources (h + 1) fl ndh.2 (Some h) ∗ 
        ith_read i r (h + 1) ∗ br_lb b0
    }}}
      ! #BeingRead @ CannotFork; NotStuck; τ; ⊤
    {{{ (pbr: loc), RET #pbr; 
            dequeue_resources (h + 1) fl ndh.2 (if (decide (pbr = ph)) then Some h else None) ∗
            (⌜ pbr = ph ⌝ ∨ 
             ⌜ pbr ≠ ph ⌝ ∗ hn_interp (ph, ndh) ∗ PE ndh.1) ∗
            ∃ b' ndbr', br_lb b' ∗ ⌜ b0 <= b' ⌝ ∗ ith_node b' (pbr, ndbr')
    }}}.
  Proof using PERS_PE.
    iIntros (Φ) "(#INV & #HTH & DR & #READ & #BR0) POST".
    iApply wp_atomic.
    iMod (check_BR_vs with "[$] [$] [$] [$] [$]") as "(%pbr & BR & BR')"; try done.
    iModIntro. 
    iApply sswp_pwp; [done| ]. 
    iApply (wp_load with "BR"). iIntros "!> BR".
    iModIntro. iApply wp_value.
    iMod ("BR'" with "[$]") as "X". 
    iApply "POST". by iFrame.
  Qed.

  Lemma read_FL_pwp_spec τ h fl nd od:
  {{{ queue_inv PE ∗ dequeue_resources h fl nd od }}}
    ! #FreeLater @ CannotFork; NotStuck; τ; ⊤
  {{{ (pfl: loc), RET (#pfl);
      ∃ ndfl, ith_node fl (pfl, ndfl) ∗ dequeue_resources h fl nd od }}}.
  Proof using.
    iIntros (Φ) "(#INV & DR) POST".
    iApply wp_atomic.
    iMod (read_FL_vs with "[$] [$]") as "(%pfl & FL & FL')". iModIntro. 
    iApply sswp_pwp; [done| ].     
    iApply (wp_load with "FL"). iIntros "!> FL".
    iModIntro. iApply wp_value.
    iMod ("FL'" with "[$]") as "X".
    iModIntro. iApply "POST". iFrame. 
  Qed.

  Lemma get_to_free_pwp_spec 
    τ h fl (ph: loc) ndh i r b
    (READ_BOUND: r <= h):
    {{{ queue_inv PE ∗
        ith_node h (ph, ndh) ∗
        dequeue_resources (h + 1) fl ndh.2 (Some h) ∗ 
        ith_read i r (h + 1) ∗
        br_lb b ∗ (⌜ b < r ⌝ -∗ (ith_rp i rs_canceled ∨ ith_rp i rs_aborted ∨ ith_rp i (rs_proc (Some rsp_completed))))
    }}}
      get_to_free q_sq #ph @ CannotFork; NotStuck; τ; ⊤
    {{{ (to_free: loc), RET #to_free;
        ∃ hn fl', hn_interp (to_free, hn) ∗ 
                    dequeue_resources (h + 1) fl' ndh.2 None ∗ PE ndh.1 }}}.
  Proof using PERS_PE.
    simpl.
    iIntros (Φ) "(#INV & #HTH & DR & #READ & #BR0 & #NO_FL) POST".
    rewrite /get_to_free. 
    pwp_pure_steps. 
    
    wp_bind (! _)%E. 
    iApply (check_BR_pwp_spec with "[-POST]").
    { apply READ_BOUND. }
    { iFrame "#∗". }
    iIntros "!> %pbr (DR & RES)".
    iDestruct "RES" as "(RES & %b1 & %ndbr1 & #BR1 & %LEb & #BRTH1)". 

    wp_bind (_ = _)%E.
    iApply sswp_pwp; [done| ].
    iApply sswp_pure_step.
    { set_solver. }
    iModIntro. iApply wp_value.

    iDestruct "RES" as "[%PTR_EQ | (%NEQ & HNI & PEv)]".
    2: { rewrite bool_decide_false; [| set_solver]. rewrite decide_False; [| done].
         pwp_pure_steps. iApply "POST". iFrame. }

    rewrite PTR_EQ. 

    rewrite bool_decide_true; [| set_solver]. rewrite decide_True; [| done].
    pwp_pure_steps.

    iModIntro. 
    wp_bind (! _)%E.
    iApply (read_FL_pwp_spec with "[-POST]").
    { iFrame "#∗". }
    iIntros "!> %pfl (%ndfl & #FLTH & DR)".

    wp_bind (Rec _ _ _)%E. pwp_pure_steps.

    wp_bind (_ <- _)%E.
    iApply wp_atomic.
    iMod (upd_fl_vs with "[$] DR HTH READ BR0 NO_FL [$] [$] [$]") as "(FL & FL')"; eauto.
    iModIntro. 
    iApply sswp_pwp; [done| ].    
    iApply (wp_store with "FL"). iIntros "!> FL".
    iModIntro. iApply wp_value.
    iMod ("FL'" with "[$]") as "(%hn & %fl' & HNF & DR & PEh)".
    iModIntro. wp_bind (Rec _ _ _)%E. pwp_pure_steps.
    iApply "POST". iFrame.
  Qed.

  Lemma free_el_pwp_spec τ ptr nd:
    {{{ hn_interp (ptr, nd) }}}
      free_el #ptr @ CannotFork; NotStuck; τ; ⊤
    {{{ v, RET v; True }}}.
  Proof using.
    simpl. iIntros (Φ) "HN POST".
    destruct nd. iDestruct "HN" as "[L0 L1]". 
    rewrite /free_el.
    pwp_pure_steps.

    wp_bind (_ +ₗ _)%E.
    iApply sswp_pwp; [done| ].
    iApply sswp_pure_step; [done| ].
    iModIntro. rewrite loc_add_0. iApply wp_value.

    iModIntro. 
    wp_bind (Free _)%E.
    iApply sswp_pwp; [done| ].
    iApply (wp_free with "L0"). iIntros "!> FREE0". 
    iModIntro. iApply wp_value.
 
    wp_bind (Rec _ _ _)%E. pwp_pure_steps. 

    wp_bind (_ +ₗ _)%E.
    iApply sswp_pwp; [done| ].
    iApply sswp_pure_step; [done| ].
    iModIntro. iApply wp_value.

    iModIntro. 
    wp_bind (Free _)%E.
    iApply sswp_pwp; [done| ].
    iApply (wp_free with "[$]"). iIntros "!> FREE1". 
    iModIntro. iApply wp_value.

    by iApply "POST". 
  Qed.
  
  Lemma dequeue_pwp_spec (τ: locale heap_lang):
    {{{ queue_inv PE ∗ dequeue_token }}}
      dequeue q_sq #() @ CannotFork; NotStuck; τ; ⊤
    {{{ (v: val), RET v; dequeue_token ∗
                         (⌜ v = NONEV ⌝ ∨ ∃ v', ⌜ v = SOMEV v' ⌝ ∗ PE v') }}}.
  Proof using PERS_PE.
    simpl. iIntros (Φ) "(#INV & TOK) POST".
    rewrite /dequeue. 
    pwp_pure_steps.

    wp_bind (! _)%E.
    iApply wp_atomic.
    iMod (dequeuer_read_head_vs with "[$] [$]") as "(%h & %ph & %fl & HEAD & HEAD')".
    iModIntro.
    iApply sswp_pwp; [done| ].
    iApply (wp_load with "HEAD"). iIntros "!> HEAD".
    iModIntro. iApply wp_value.
    iMod ("HEAD'" with "[$]") as "DR". iModIntro. 
    
    wp_bind (Rec _ _ _)%E. pwp_pure_steps. 
    
    wp_bind (! _)%E.
    iApply wp_atomic.
    iMod (dequeuer_read_tail_vs with "[$] [$]") as "(%t & %br & %pt & TAIL & %ORDER & #HT & TAIL')".
    iModIntro. 
    iApply sswp_pwp; [done| ].
    iApply (wp_load with "[$]"). iIntros "!> TAIL".
    iModIntro. iApply wp_value.
    iMod ("TAIL'" with "[$]") as "DR".
    iModIntro. 

    wp_bind (_ = _)%E.
    iApply sswp_pwp; [done| ].
    iApply sswp_pure_step.
    { set_solver. }
    iModIntro. iApply wp_value.

    iDestruct "HT" as "[[%GE <-] | (%NEMPTY & %ndh & #HTH)]". 
    { assert (t = h) as ->.
      { red in ORDER. lia. }
      rewrite bool_decide_true; [| done].
      iNext.
      iApply fupd_wp.
      iMod (dequeuer_close with "[$] [$]") as "TOK".
      pwp_pure_steps. 
      iApply "POST". iFrame.
      by iLeft. }

    iNext.
    rewrite bool_decide_false; [| set_solver]. pwp_pure_steps.
    wp_bind (get_val _)%E.
    iApply (get_head_val_pwp_spec with "[-POST]").
    { done. }
    { iFrame "#∗". }
    iIntros "!> (DR & PEh)".
    wp_bind (Rec _ _ _)%E. pwp_pure_steps.

    destruct ndh as [vh nxh]. simpl.
    wp_bind (_ <- _)%E.
    iApply (update_ohv_pwp_spec with "[$INV $PEh]").
    iIntros "!> PH".
    wp_bind (Rec _ _ _)%E. pwp_pure_steps.

    wp_bind (get_next _)%E.
    iApply (get_head_next_pwp_spec with "[-POST]").
    { iFrame "#∗". }
    iIntros "!> DR". simpl.

    wp_bind (_ <- _)%E.
    iApply (dequeue_upd_head_pwp_spec with "[$INV $HTH $DR]").
    iIntros "!> (DR & (%i & %r & %b & #ITHR & %BR & #BRB & #TOKS))".

    wp_bind (Rec _ _ _)%E. do 2 pwp_pure_step.
    wp_bind (get_to_free _ _)%E.         
    iApply (get_to_free_pwp_spec with "[-POST]").
    { apply BR. }
    { iFrame "#∗". }
    
    iIntros "!> %to_free (%hfr & %fl' & HNFR & DR & PEv)".
    wp_bind (Rec _ _ _)%E. pwp_pure_steps.
    wp_bind (free_el _)%E.
    iApply (free_el_pwp_spec with "[HNFR]").
    { iFrame. }
    iIntros "!> %v _".

    wp_bind (Rec _ _ _)%E. do 2 pwp_pure_step. 
    iClear "TOKS ITHR HTH". 

    iClear "BRB". 
    clear dependent r t hfr to_free br ph b. 

    iApply fupd_wp.
    iMod (dequeuer_close with "[$] [$]") as "TOK". 
    pwp_pure_steps. iApply "POST". iFrame.
    iRight. iExists _. iSplit; [done| ]. by iFrame. 
  Qed.
    
End DequeuePwp.
