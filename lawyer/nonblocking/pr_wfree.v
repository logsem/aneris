...
Section WaitFreePR.
  ...
  (* TODO: move *)
  Definition wptp_from_gen {Σ: gFunctors}
  ...
  Definition wptp_wfree {Σ} {Hinv : @IEMGS _ _ HeapLangEM EM Σ}
    ...
    iProp Σ :=
    wptp_gen (fun e τ Φ => if decide (τi = τ) 
                          then wp_tc s e (ii <? trace_length etr) Φ
                          else
                            let _ := iris_OM_into_Looping in
                            pwp s ⊤ τ e Φ)
      (trace_last etr).1 Φs.
  ...
  (* TODO: need to support the situation where 1/2 of τ's phase is taken out *)
  (*      already in the starting state - use a token? *)
