...
Record WaitFreeSpecToken (MS: gmultiset val) := {
  ...
  wfst_preG: gFunctors -> Type;
  wfst_G: gFunctors -> Type;
  wfst_Σ: gFunctors;
  ...
  wfst_init_mod `{heap1GS Σ, invGS_gen HasNoLc Σ, MethodTokenPre Σ, wfst_preG Σ}:
    forall c (INIT: wfst_is_init_st c),
      ⊢ hl_phys_init_resource c ={⊤}=∗ ∃ (_: MethodToken MS Σ) (wG: wfst_G Σ), 
                                     wfst_mod_inv (wG := wG) ∗ methods_toks MS;
  wfst_F: nat;
  wfst_spec:
  forall {M: Model} {EM: ExecutionModel heap_lang M} {Σ} {OHE: OM_HL_Env OP_HL_WF EM Σ}
    {MT: MethodToken MS Σ} {wG: wfst_G Σ},
    (let _: heap1GS Σ := iem_phys HeapLangEM EM in wfst_mod_inv (wG := wG) ) ⊢
    [∗ set] m ∈ dom MS, method_spec_token MS wfst_F m;
    
  wfst_safety_spec:
    ∀ {Σ : gFunctors} `{heap1GS Σ, invGS_gen HasNoLc Σ, MethodToken MS Σ} {wG: wfst_G Σ},
      wfst_mod_inv (wG := wG) ⊢ [∗ set] m ∈ dom MS, token_safety_spec m;
}.
