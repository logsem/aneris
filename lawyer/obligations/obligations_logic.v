...
Section ProgramLogic.
  ...
  Section BOU.
    ...
    Lemma BOU_AMU__f_strong E ζ ζ' π Q:
      ⊢
        BOU E LIM_STEPS (∃ d R1 R2, cp π d ∗ obls ζ (R1 ∪ R2) ∗ th_phase_eq ζ π ∗ Q R1 R2 ∗ ⌜ R1 ## R2 ⌝) -∗
        AMU__f E ζ ζ' obls_act (∃ π1 π2 R1 R2,
            th_phase_eq ζ π1 ∗ obls ζ R1 ∗ 
            th_phase_eq ζ' π2 ∗ obls ζ' R2 ∗
            ⌜ phase_lt π π1 /\ phase_lt π π2 ⌝ ∗ Q R1 R2 ∗ ⌜ R1 ## R2 ⌝) (aeGS := oGS').
    Proof using.
      clear H1 H0 H. 
      ...
      { eauto. }
      iDestruct "X" as (δ'' π1 π2 R1 R2) "(SI & OB1 & PH1 & OB2 & PH3 & ((%PH1 & %PH2) & %TRANS & Q & ?))".
    ...
    Lemma BOU_AMU__f'_strong E ζ ζ' π Q P:
      ⊢
        let om_fork_post := ∃ π1 π2 R1 R2,
            th_phase_eq ζ π1 ∗ obls ζ R1 ∗ 
            th_phase_eq ζ' π2 ∗ obls ζ' R2 ∗
            ⌜ phase_lt π π1 /\ phase_lt π π2 ⌝ ∗ Q R1 R2 ∗ ⌜ R1 ## R2 ⌝ in
        BOU E LIM_STEPS ((om_fork_post -∗ P) ∗ ∃ d R1 R2, cp π d ∗ obls ζ (R1 ∪ R2) ∗ th_phase_eq ζ π ∗ Q R1 R2 ∗ ⌜ R1 ## R2 ⌝) -∗
        AMU__f E ζ ζ' obls_act P (aeGS := oGS').
    Proof using.
      simpl. iIntros "BOU".
      rewrite /AMU__f.  
      iApply AMU_impl_wand.
      2: { iApply (BOU_AMU__f_strong with "[BOU]").
    ...
    Proof using.
      clear H1 H0 H.
      iIntros "BOU".
      iApply (BOU_AMU__f'_strong _ _ _ _ (fun R1 R2 => ( ⌜ R1 = R0 ∖ R' /\ R2 = R0 ∩ R' ⌝)%I)).
      iApply (BOU_wand with "[] [$]"). iIntros "(P & (%&?&?&?))".
      iFrame. iSplitR.
      2: { do 2 iExists _. iSplitL.
